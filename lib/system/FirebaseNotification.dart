...
// initialise the plugin. app_icon needs to be a added as a drawable resource to the Android head project
      ...
      onResume: (Map<String, dynamic> message) async {
        ...
        if (message != null) {
          ...
          await navigatorKey.currentState!.pushReplacement(
            MaterialPageRoute(
              builder: (context) => FrontPageView(payload: selectedNotificationPayload),
            ),
          );
        }
  ...
  sendNotification({String? title, String? body, Map? data}) async {
    var androidPlatformChannelSpecifics =
        AndroidNotificationDetails(channelId, channelName, channelDescription, importance: Importance.max, priority: Priority.high, ticker: 'ticker');
    var iOSPlatformChannelSpecifics = IOSNotificationDetails();
    var platformChannelSpecifics = NotificationDetails(android: androidPlatformChannelSpecifics, iOS: iOSPlatformChannelSpecifics);

    final payload = json.encode(data);
    await flutterLocalNotificationsPlugin.show(111, title, body, platformChannelSpecifics, payload: payload.toString());
  }
}
